...
* **Limitações:** Os resultados da Random Forest foram elevados; para reduzir risco de overfitting e vazamento, adotamos pipelines com split antes de qualquer transformação, validação cruzada e avaliação final em conjunto de teste segregado. Em séries temporais, evitamos k-fold aleatório e utilizamos backtesting com janelas rolantes. Ainda assim, o histórico curto e a ausência de variáveis externas podem limitar a capacidade preditiva do Prophet em mudanças estruturais.

* **Próximos passos:** Expandir a série histórica, incorporar variáveis exógenas (campanhas de saúde, sazonalidades regionais, disponibilidade hospitalar), testar modelos adicionais (XGBoost/CatBoost; ensembles) e realizar feature importance e SHAP para interpretar os drivers de valor de venda.
